...
static inline void round_tiger(uint64_t *restrict v1, uint64_t *restrict v2, uint64_t *restrict v3, const uint64_t word, const uint8_t mul) {
	*v3 ^= word;
    *v1 -= table[((uint8_t)(*v3)) & 0xFF] ^ table[256 + (((uint8_t)(*v3 >> 16)) & 0xFF)] ^ table[512 + (((uint8_t)(*v3 >> 32)) & 0xFF)] ^ table[768 + (((uint8_t)(*v3 >> 48)) & 0xFF)];
    *v2 += table[768 + (((uint8_t)(*v3 >> 8)) & 0xFF)] ^ table[512 + (((uint8_t)(*v3 >> 24)) & 0xFF)] ^ table[256 + (((uint8_t)(*v3 >> 40)) & 0xFF)] ^ table[((uint8_t)(*v3 >> 56)) & 0xFF];
    *v2 *= mul;
}

static inline void pass(uint64_t *restrict A, uint64_t *restrict B, uint64_t *restrict C, const uint64_t *words, const uint8_t mul) {
	round_tiger(A, B, C, words[0], mul);
    round_tiger(B, C, A, words[1], mul);
    round_tiger(C, A, B, words[2], mul);
	round_tiger(A, B, C, words[3], mul);
    round_tiger(B, C, A, words[4], mul);
	round_tiger(C, A, B, words[5], mul);
    round_tiger(A, B, C, words[6], mul);
	round_tiger(B, C, A, words[7], mul);
}

static inline void key_schedule(uint64_t *words) {
...
void tiger(const uint8_t *restrict buffer, uint64_t *restrict words, const uint32_t nb_read_bytes, const uint32_t buffer_size, struct function_infos *restrict func) {
	...
	for(uint16_t block_index = 0; block_index < nb_blocks_in_buffer; block_index++) {
		...
		func->md.hash_values_u64[0] ^= AA;
      	
		func->md.hash_values_u64[1] -= BB;

		func->md.hash_values_u64[2] += CC;
	}
}
